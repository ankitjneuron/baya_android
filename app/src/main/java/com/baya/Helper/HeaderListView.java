...
public class HeaderListView extends RelativeLayout {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		// The list view's scroll bar can be hidden by the header, so we display
		// our own scroll bar instead
	...
	private class HeaderListViewOnScrollListener implements AbsListView.OnScrollListener {
		...
		public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
			...
			if (totalItemCount > 0 && previousFirstVisibleItem != realFirstVisibleItem) {
				...
				boolean needNoHeaderUpToHeader = currIsLast && currHasHeader && !nextHasHeader && realFirstVisibleItem == firstVisibleItem
						&& Math.abs(mListView.getChildAt(0).getTop()) >= mListView.getChildAt(0).getHeight() / 2;
			...
			if (scrollingStart) {
				int scrolled = realFirstVisibleItem >= firstVisibleItem ? mListView.getChildAt(realFirstVisibleItem - firstVisibleItem)
						.getTop() : 0;
				...
				int headerH = doneMeasuring ? (prevH - nextH) * direction * Math.abs(scrolled) / (direction < 0 ? nextH : prevH)
						+ (direction > 0 ? nextH : prevH) : 0;
			...
			if (noHeaderUpToHeader) {
				...
				mHeader.scrollTo(0, mHeader.getLayoutParams().height
		...
		private void setMeasurements(int realFirstVisibleItem, int firstVisibleItem) {
			...
			if (direction > 0) {
				nextH = realFirstVisibleItem >= firstVisibleItem ? mListView.getChildAt(realFirstVisibleItem - firstVisibleItem)
						.getMeasuredHeight() : 0;
			}
		...
		private void addSectionHeader(int actualSection) {
			...
			if (mAdapter.hasSectionHeaderView(actualSection)) {
				...
				mHeaderConvertView.measure(MeasureSpec.makeMeasureSpec(mHeader.getWidth(), MeasureSpec.EXACTLY),
